...
- Remove parentheticals like `(Fast mode)` (case-insensitive) and append `-fast` to the base name: `Claude 4.6 Opus (Fast mode)` → `claude-4.6-opus-fast`
- For Claude rows that put the family before the version, normalize to the canonical version-before-family ID: `Claude Opus 4.7 (fast mode)` → `claude-4.7-opus-fast`
