...
- Normalize Anthropic rows where the Claude family appears before the version so fast-mode rows share the same base ID as the standard model: `Claude Opus 4.7 (fast mode)` → `claude-4.7-opus-fast`
