...
Fetch the raw markdown. As of 2026-08-23 the page has **two** tables with columns: Model | Provider | Input | Cache write | Cache read | Output | Notes:
...
Auto Cost is not a markdown table row on `models-and-pricing.md`. The rendered HTML page includes an Auto Cost card/table (Name | Input | Cache Write | Cache Read | Output, currently $6) and the page references `modelId` `auto-cost`. Output prices also live in a hashed Next.js chunk under `docs-static/_next/static/chunks/*.js`.
...
2. Download chunks and find the pricing catalog (search for `id:"auto-cost"` or `id:"grok-4.6"` together with `output:`). The chunk hash changes over time (2026-08-23: still `0p1ggu34p8cgd.js`; was `0qfcspezy5-zl.js` on 2026-08-21).
...
The rendered HTML Auto Cost table and Teams pricing markdown (`https://cursor.com/docs/account/teams/pricing.md`, Auto Cost row `$1.25 / $1.25 / $0.25 / $6`) are cross-checks only. Source of truth for this repo is the models-and-pricing.md tables plus catalog Auto Cost.
...
- Typical size: markdown row count + `auto` (e.g. 47 + 1 = 48 as of 2026-08-23)
