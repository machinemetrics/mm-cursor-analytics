...
- For Anthropic display names in `Claude <version> <family>` order, keep the normal lowercased/spaced ID: `Claude 4.6 Opus` → `claude-4.6-opus`
- For Anthropic display names in `Claude <family> <version>` order where the family comes before the version, use Cursor's hyphenated version slug: `Claude Opus 4.8` → `claude-opus-4-8`
