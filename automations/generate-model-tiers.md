...
Fetch the raw markdown. As of 2026-09-07 the page has **two** tables with columns: Model | Provider | Input | Cache write | Cache read | Output | Notes:

- **Cursor Models** (first-party): Grok 4.6, Grok 4.5, Composer 2.5, and their Fast variants
- **Other Models** under `### Model pricing`: third-party models (including Gemini 3.8 Flash and Claude Fable 5.1)
...
`auto-cost` is not a markdown table row on `models-and-pricing.md`. As of 2026-09-07 the catalog names that entry **Legacy Enterprise Auto** (hidden). The rendered HTML shows a Legacy Enterprise Auto table (`$1.25 / $1.25 / $0.25 / $6`) and still references `modelId` `auto-cost`. Individual Auto modes now bill at the list price of the routed model — still emit `auto` because Cursor stores Auto as `"default"` and the extension maps that to `"auto"`.
...
2. Download chunks and find the pricing catalog (search for `id:"auto-cost"` or `id:"grok-4.6"` together with `output:`). The chunk hash changes over time (2026-09-07 through 2026-09-05: `15gbgx.vwkhtn.js`; 2026-09-04 and 2026-09-03: `0-fi77m__78_w.js`; 2026-09-02: `0~ms10zi70-sr.js`; 2026-09-01 through 2026-08-25: `07q5_bowxn4is.js`; was `0p1ggu34p8cgd.js` on 2026-08-23; was `0qfcspezy5-zl.js` on 2026-08-21).
...
Use **display-name normalization** for keys in `model_tiers.json` (not raw catalog `id` / `slug`). The catalog mixes conventions (`claude-4-6-opus` vs `claude-opus-4-6`, `gpt-5.1` id with slug `gpt-5`); state.vscdb historically matches dotted display names such as `claude-4.6-opus` and `gpt-5`. Catalog ids may use hyphens in versions (`claude-fable-5-1`); still emit the dotted display-name id (`claude-fable-5.1`).
...
- Keep `.` in version numbers (e.g. `4.6` stays `4.6`, not `4-6`; `5.1` stays `5.1`)
...
- Normalize Anthropic names literally from display order. Newer names flip family/version (`Claude Opus 4.8`, `Claude Sonnet 5`, `Claude Fable 5.1`) vs older `Claude 4.6 Opus` style:
...
Write to `model_tiers.json` in the repo root. Prefer compact one-line JSON entries. Each model must include both `tier` and `output` (output price per 1M tokens) so the extension can compute effective cost when Max Mode is enabled:
...
- Claude Fable 5 ($50) and Claude Fable 5.1 ($50) are "extremely expensive"
- Claude Sonnet 5 ($10) is "daily driver"
- Gemini 3.7 Flash ($3.5) and Gemini 3.8 Flash ($3.5) are "cheap"
...
- Typical size: markdown row count + `auto` (e.g. 49 + 1 = 50 as of 2026-09-07)
