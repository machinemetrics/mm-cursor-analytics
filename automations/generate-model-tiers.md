...
- Newer Claude Opus display names that use the `Claude Opus N.N` word order are stored by Cursor with the version segment hyphenated. Use that stored form: `Claude Opus 4.8` → `claude-opus-4-8`; `Claude Opus 4.7 (fast mode)` → `claude-opus-4-7-fast`
